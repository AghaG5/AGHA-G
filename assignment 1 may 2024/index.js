...
let maxScore = 20;
let minScore = 5;
function countbelowandexceed(scores) {
    let maxCount = 0;
    let minCount = 0;
...
//Question 16
function removeFalse(falsey) {
    return falsey.filter((value) => !!value);
}
const arraywithfalseyvalue = [null, 0, "", undefined, NaN, "hello", 35];
...
const EvenOddElement = [1, 2, 3, 4, 5, 6, 7];
...
//Question 19
function findIndexArray(array, Element) {
...
//Question 20
function findSmallestnumber(no) {
...
const number1 = [5, 3, 4, 1, 2];
const smallestnumber = findSmallestnumber(number1);
console.log("smallest number is =", smallestnumber);
//Question 21
function calculateProduct(numbers1) {
    return number1.map((no1) => no1 * no1);
}
const numbers1 = [5, 4, 3, 2, 1];
const no1 = calculateProduct(numbers1);
console.log("product is", no1);
//Question 22
function filterByLength(strings, n) {
    return strings.filter((str) => str.length > n);
}
const word = ["apple", "banana", "orange", "mango", "kiwi"];
const filteredwords = filterByLength(word, 3);
console.log(filteredwords);
//Question 23
function findDuplicates(numberss1) {
    const duplicates = {};
    numberss1.forEach((no2) => {
...
const numberss1 = [1, 2, 5, 3, 3, 4, 5];
findDuplicates(numberss1);
//Question 24
function IncreamentAll(numarr) {
    return numarr.map((inc) => ++inc);
}
const arrOfNum = [1, 2, 3];
const inc = IncreamentAll(arrOfNum);
console.log("normal arr", arrOfNum);
console.log("inc arr", inc);
//Question 25
function countOccurrences(counts) {
    const occurrences = {};
    counts.forEach((nums1) => {
...
const UnCountednumbers = [1, 1, 2, 3, 3, 4, 5, 5];
const cntnumbers = countOccurrences(UnCountednumbers);
console.log(cntnumbers);
//Question 26
function isStored(arry) {
...
const namesArray = ["bob", "charlie", "david", "angel", "goat"];
const formatednames = formatnames(namesArray);
console.log(formatednames);
//Question 28
function converfahrenheitintocelsius(giventemprature) {
    const celsiustemp = giventemprature.map((temp) => ((temp - 32) * 5) / 9);
    console.log("celsius temprature:");
    celsiustemp.forEach((temp) => console.log(temp));
}
const fahrenheittemp = [32, 35, 53, 68, 120];
converfahrenheitintocelsius(fahrenheittemp);
//Question 29
function minMaxAverage(number2) {
    if (number2.length === 0) {
        console.log("Array must not be empty");
    }
    const min = Math.min(...number2);
    const max = Math.max(...number2);
    const sum = number2.reduce((acr, cur) => acr + cur, 0);
    const average = sum / number2.length;
    return { min, max, average };
}
const numbarray = [5, 10, 20, 25, 30];
const result = minMaxAverage(numbarray);
console.log(result);
//Question 30
function swapIndex(values, index3, index4) {
...
//Question 31
function countCharacter(strr, char) {
    let count = 0;
...
// Question 34
function logarrayinreverse(arr1) {
